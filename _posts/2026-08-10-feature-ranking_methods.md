...
Model performance is first measured using the original dataset:

$$
Score_{original}
$$

The feature $$X_i$$ is then randomly shuffled and model performance is measured again:
...
$$
Importance(X_i) = Score_{original} - Score_{permuted}
$$

where:

- $$Importance(X_i)$$ = Importance score of feature $$X_i$$
- $$Score_{original}$$ = Model performance before permutation
- $$Score_{permuted}$$ = Model performance after feature permutation

Features causing larger performance degradation receive higher importance scores.
...
|------------|------------|
| Model-agnostic and easy to apply | Computationally expensive for large datasets |
| Captures nonlinear relationships | Requires a trained model |
| Reflects true impact on prediction performance | Correlated features may receive lower importance |
| Easy to interpret | Importance depends on model quality |
| Works with any machine learning model | Repeated permutations may be required for stability |
| Provides realistic feature rankings | Computational cost grows with feature count |
